...
def main(db, butler, skyMapPolys, layer="", margin=10, verbose=True, visit=None):
    checkSql = "SELECT COUNT(*) FROM overlaps WHERE visit=? AND ccd=?"
    insertSql = "INSERT INTO overlaps (tract, patch, visit, ccd, filter, layer) VALUES (?, ?, ?, ?, ?, ?)"
    if visit is None:
    ...
    for dataRef in dataRefs:
        ...
        cursor = db.execute(checkSql, (visit, ccd))
        if cursor.fetchone()[0]:
            if verbose:
                print("Skipping visit=%d, ccd=%d: already present in overlap table." % (visit, ccd))
            continue
        if not dataRef.datasetExists("calexp"):
            if verbose:
                print("Skipping visit=%d, ccd=%d: no calexp found." % (visit, ccd))
            continue
        ...
        for tract, patches in skyMapPolys.findOverlaps(bbox, wcs, margin=margin):
            print("Adding patches for visit=%d, ccd=%d, tract=%d to overlap table." %
                  (visit, ccd, tract))
            #live with intermittent locking of the db....
            while True:
                try:
                    db.executemany(insertSql, [(tract, str(patch), visit, ccd, filter, layer) for patch in patches])
                    break
